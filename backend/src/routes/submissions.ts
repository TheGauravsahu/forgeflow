...
// POST /api/submissions/submit/:formId (Submit answers - public)
...
// GET /api/submissions/:formId (List submissions for a form - authenticated)
router.get('/:formId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
...
// GET /api/submissions/:formId/analytics (Get submission analytics for a form - authenticated)
router.get('/:formId/analytics', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) return res.status(401).json({ error: 'Unauthorized.' });
  const { formId } = req.params;
